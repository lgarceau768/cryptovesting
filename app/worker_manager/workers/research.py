...
logger = Logger.LogManager("researchWorker_"+str(sys.argv[1]))
...
logger.log('Created the contract object to uniswap factory: '+uniswap_factory, level="INFO")
...
logger.log('Created the contract object to pancake router: '+panRouterContractAddress, level="INFO")
...
def handle_event(event):
   ...
   # print("Block: " + block)
    ...
    if(token0.upper().strip('"') == wbnb2 and token1.upper().strip('"') == tokenToBuy2):
        print("Mint="+tokenToBuy2, flush=True)
...
def main():
    ...
    try:
        ...
        logger.log('Research loop exception: '+str(e), level="DEBUG")
